...
export const errorHandler: ErrorRequestHandler = (err: any, req: Request, res: Response, _next: NextFunction) => {
  switch (err.status) {
    ...
    case 403:
      logger.warn(`403 error detected: ${err.message}`);
      res.status(403);
      res.render('errors/forbidden');
      break;

    case 404:
      logger.warn(`404 error detected for ${req.originalUrl}, rendering not-found page`);
      res.status(404);
      res.render('errors/not-found');
      break;

    case 405:
      logger.warn(`405 error detected: ${err.message}`);
      res.status(405);
      res.render('errors/forbidden');
      break;
