...
//     try {
//         const previewData: ViewDTO = await req.swapi.getImportPreview(
//             currentDataset.id,
//             currentRevision.id,
...
// function updateCurrentImport(currentImport: FileImportDTO, dimensionCreationRequest: SourceAssignmentDTO[]) {
...
//     const dimensionCreationRequest: SourceAssignmentDTO[] = currentFileImport.sources.map((source) => {
//         return {
//             sourceId: source.id,
...
//     try {
//         const dataset: DatasetDTO = await req.swapi.assignSources(
//             currentDataset.id,
//             currentRevision.id,
//             currentFileImport.id,
...
//         await req.swapi.updateDatasetInfo(datasetId, infoDto);
//         res.redirect(req.buildUrl(`/publish/${dataset.id}/tasklist`, req.language));
//     } catch (err) {
//         logger.error(`Something went wrong trying to load the title: ${err}`);
//         res.status(404);
