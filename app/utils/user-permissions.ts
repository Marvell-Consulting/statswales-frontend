...
import type { UserGroupWithRolesDTO } from '../dtos/user/user-group-with-roles-dto';
import { GroupRole } from '../../app/enums/group-role';
...
export const isApprover = (user: UserDTO): boolean => {
  return (
    user.groups?.some((g: UserGroupWithRolesDTO) => g.roles.includes(GroupRole.Approver)) || false
  );
};
...
export const isApproverForDataset = (user: UserDTO, dataset: DatasetDTO): boolean => {
  ...
  return getApproverUserGroups(user).some(
    (g: UserGroupWithRolesDTO) => g.group.id === dataset.user_group_id
  );
};
