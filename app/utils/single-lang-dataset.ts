...
export const singleLangRevision = (
  revision?: RevisionDTO,
  lang?: string
): SingleLanguageRevision | undefined => {
  ...
  return {
    ...
    providers: (revision.providers || []).filter(
      (provider) => provider.language === lang?.toLowerCase()
    ),
  ...
  return {
    ...
    measure: dataset.measure
      ? {
          ...
          measure_table: dataset.measure.measure_table?.filter(
