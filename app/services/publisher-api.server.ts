...
import type { RevisionMetadataDTO } from '../dtos/revision-metadata';
import type { DataTableDto } from '../dtos/data-table';
import type { SourceAssignmentDTO } from '../dtos/source-assignment-dto';
import { logger as parentLogger } from '../utils/logger.server';
...
import type { DimensionPatchDTO } from '../dtos/dimension-patch-dto';
import type { DimensionDTO } from '../dtos/dimension';
import type { DimensionMetadataDTO } from '../dtos/dimension-metadata';
...
import type { UserGroupMetadataDTO } from '../dtos/user/user-group-metadata-dto';
import type { UserGroupListItemDTO } from '../dtos/user/user-group-list-item-dto';
import type { FileImportDto } from '../dtos/file-import';
import type { UserDTO } from '../dtos/user/user';
import type { UserCreateDTO } from '../dtos/user/user-create-dto';
import type { AvailableRoles } from '../interfaces/available-roles';
import type { RoleSelectionDTO } from '../dtos/user/role-selection-dto';
import { UserStatus } from '../enums/user-status';
import { AuthProvider } from '../enums/auth-providers';
import { TaskDTO } from '../dtos/task';
import type { TaskDecisionDTO } from '../dtos/task-decision';
...
export class PublisherApi {
  ...
  constructor(private lang = Locale.English, private token?: string) {
    this.lang = lang;
    this.token = token;
  }
  ...
  public async getOriginalUploadDimension(
    datasetId: string,
    dimensionId: string
  ...
  public async uploadCSVtoCreateDataset(
    file: Blob,
    filename: string,
    title: string
  ): Promise<DatasetDTO> {
  ...
  public async addDatasetProvider(
    datasetId: string,
    provider: RevisionProviderDTO
  ): Promise<DatasetDTO> {
    return this.fetch({
  ...
  public async updateAssignedProviders(
    datasetId: string,
  ...
  public async getAllProviders(): Promise<ProviderDTO[]> {
    ...
    return this.fetch({ url: 'provider' }).then(
      (response) => response.json() as unknown as ProviderDTO[]
    );
  }
  ...
  public async updatePublishDate(
    datasetId: string,
    revisionId: string,
    publishDate: string
  ): Promise<DatasetDTO> {
  ...
  public async getAllOrganisations(): Promise<OrganisationDTO[]> {
    ...
    return this.fetch({ url: 'organisation' }).then(
      (response) => response.json() as unknown as OrganisationDTO[]
    );
  }
  ...
  public async updateUserGroup(group: UserGroupDTO): Promise<UserGroupDTO> {
    ...
    return this.fetch({
      ...
      method: HttpMethod.Patch,
      json: group
    }).then((response) => response.json() as unknown as UserGroupDTO);
  }
  ...
  public async getAvailableUserRoles(): Promise<AvailableRoles> {
    ...
    return this.fetch({ url: `admin/role` }).then(
      (response) => response.json() as unknown as AvailableRoles
    );
  }

  public async updateUserRoles(
    userId: string,
    selectedRoles: RoleSelectionDTO[]
  ): Promise<UserDTO> {
    ...
    return this.fetch({
      ...
      method: HttpMethod.Patch,
      json: selectedRoles
    }).then((response) => response.json() as unknown as UserDTO);
  }
